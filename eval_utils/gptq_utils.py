    ...
    def fasterquant(
        ...
        for i1 in range(0, self.columns, blocksize):
            ...
            for i in range(count):
                w = W1[:, i]
                d = Hinv1[i, i]
                assert groupsize == 16, "For NVFP4 Quantization"
                assert not static_groups
                if groupsize != -1:
                ...
                # print(f"[debug]: scale shape: {self.quantizer.scale.shape}")
                # print(f"[debug]: weight shape: {w.unsqueeze(1).shape}")
    ...
    for i in range(len(layers)):
        ...
        for names in sequential:
            ...
            for name in subset:
                ...
                # print(f"[debug]: w groupsize = {layer_w_groupsize}")
...
def rtn_fwrd(model, dev, args, custom_layers=None):
    ...
    for i in tqdm.tqdm(range(len(layers)), desc="(RtN Quant.) Layers"):
        ...
        for name in subset:
            ...
            q, nvfp4_weight, scale = quantizer.fake_quantize(W)
            subset[name].weight.data = q.to(next(iter(layer.parameters())).dtype)
            if args.export_to_et:
                subset[name].register_buffer("nvfp4_weight", nvfp4_weight)
