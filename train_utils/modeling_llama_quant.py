...
class LlamaMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        down_proj = self.down_proj(
            self.act_fn(self.gate_proj(x, R1=self.R1_gate.weight)) * self.up_proj(x, R1=self.R1_up.weight),
            R1=self.R1_down.weight,
            RM=self.RM.weight,
            RM_hat=self.RM_hat.weight,
            transpose_R1=True,
        )
        # down_proj = self.down_proj(
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # print(self.R2)
        # end debug ===============================================
        query_states = self.q_proj(hidden_states, R1 = self.R1_q.weight)
        key_states = self.k_proj(hidden_states, R1 = self.R1_k.weight)
        ...
        #     )
        # else:
        attn_output = self.o_proj(attn_output, R1 = self.R1_o.weight, R2=self.R2.weight, transpose=True)
...
class LlamaSdpaAttention(LlamaAttention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # print(f"key_shape: {key_states.shape}")
        # print(f"value_shape: {value_states.shape}")
        # exit(0)
        ...
        # print(f"number of groups: {self.num_key_value_groups}")
        # exit(0)
        key_states = repeat_kv(key_states, self.num_key_value_groups)
        value_states = repeat_kv(value_states, self.num_key_value_groups)
        ...
        # debug: remove R2 to see the change in loss ===========================
        attn_output = self.o_proj(attn_output, R1=self.R1_o.weight, R2=self.R2_hat.weight, transpose_R1=True, transpose_R2=True)
        # attn_output = self.o_proj(attn_output)       
        # debug: print the shape of o_proj, check the effect of multi-head attention=======
        # print(self.o_proj.weight.shape)
        # end debug ========================================================================
        # attn_output = self.o_proj(attn_output, R1, transpose_R1 = True)
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        # print(f"Type of self_attention: {config._attn_implementation}!!!!!!!!!!!!!")
        # exit(0)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if R1 is not None:
            ...
            inputs_embeds = block_diag_matmul(inputs_embeds.to(torch.float64), R1.to(torch.float64)).to(
                dtype
            )
        # end debug ===================================================================
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        # debug: remove rotation to check the change in loss =====================================
        if self.R1 is not None:
            dtype = hidden_states.dtype
            hidden_states = block_diag_matmul(hidden_states.to(torch.float64), self.R1.weight.transpose(-1, -2).to(torch.float64)).to(
                dtype
            )
